...
public class Calculator extends JFrame {
	...
	public Calculator() {
		...
		//위치와 크기(x : 8 y : 10의 위치에 270X70 크기)
		inputSpace.setBounds(8, 10, 270, 70);
		
		//버튼을 만들 패널
		JPanel buttonPanel = new JPanel();
		//레이아웃 지정 - 격자형태로 배치해주는 GridLayout사용
		//GridLayout(4, 4, 10, 10) -> 가로 칸수, 세로 칸수, 좌우 간격, 상하간격
	...
	class PadActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			//=이 눌렸을 경우 위에 입력된 식을 계산, 계산값이 나오도록 함
				...
				//밑의 메소드들을 이용하여 계산, 계산식 화면에 값을 띄어줌
	...
	public double calculate(String inputText) {
		...
		//위에 메소드를 실행하면 ArrayList에 숫자와 연산 기호가 담김
		double prev = 0;
		double current = 0;
		//연산 기호에 대한 처리를 위한 변수
		String mode = "";
		
		//+일 경우 add, -일 경우 sub, x일 경우 mul, /일 경우 div
